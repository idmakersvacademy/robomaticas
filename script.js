...
const conceptByType = {
  add: {
    ...
    intro: "Cuando sumamos, unimos dos grupos de clones para saber cuantos llegan juntos al portal.",
    note: "Ejemplo: un grupo de clones se junta con otro grupo y forman una tripulacion mas grande.",
    demo: [
      ["clones", 2],
      ["symbol", "+"],
      ["clones", 3],
...
const additionMachines = [
  { name: "Estacion de energia", activity: "Junta dos cargas de energia para activar el sistema.", verb: "unio energia en" },
  { name: "Panel de estrellas", activity: "Suma estrellas para encender el panel principal.", verb: "encendio" },
  { name: "Modulo de baterias", activity: "Combina baterias y estabiliza el laboratorio.", verb: "cargo" },
  { name: "Capsulas gemelas", activity: "Une las capsulas correctas antes del glitch.", verb: "conecto" },
  { name: "Portal de sumas", activity: "Reune suficiente energia para abrir el portal.", verb: "desbloqueo" },
];
...
function startFactoryLevel(index) {
  ...
  els.factoryTitle.textContent = mode === "add" ? "Bahia de Sumas: Laboratorio de Energia" : level.title;
  els.factoryStartButton.style.display = "inline-grid";
  els.factoryStartButton.textContent = mode === "add" ? "Iniciar Sumas" : "Iniciar Laboratorio";
  els.factoryOptions.innerHTML = "";
  els.cloneRoundBrief.textContent = "El sistema espera tu ayuda!";
  els.cloneEquation.textContent = mode === "add" ? "Inicia la suma" : "Inicia el laboratorio";
  els.cloneGroups.innerHTML = "";
  setFactoryRoboMood("happy");
  setFactoryMessage(
    mode === "add"
      ? "La bahia necesita energia. Presiona Iniciar Sumas para juntar cargas con Robo."
      : "El laboratorio esta inestable. Presiona Iniciar Laboratorio para crear clones con multiplicaciones.",
  );
  updateFactoryHud();
...
function nextFactoryQuestion() {
  ...
  els.cloneEquation.classList.remove("fusion-active");
...
function createFactoryQuestion() {
  if (factoryMode() === "add") return createFactoryAdditionQuestion();
...
function createFactoryAdditionQuestion() {
  ...
  const scenarios = [
    "Junta los clones azules con los clones dorados.",
    "Conecta los clones del panel con los clones del reactor.",
    "Une las dos patrullas de Robo para cargar el portal.",
    "Reune ambos grupos brillantes antes del glitch.",
  ];
  return {
    text: `${left} + ${right} = ?`,
    brief: randomFrom(scenarios),
    equation: `${left} + ${right} = ?`,
    equationMarkup: `
      <span class="addition-clone-set">${cloneMarkup(left)}</span>
      <span class="addition-symbol">+</span>
      <span class="addition-clone-set">${cloneMarkup(right)}</span>
      <span class="addition-symbol">=</span>
...
function factoryCorrect() {
  ...
  setFactoryMessage(`Correcto! Robo ${machines[factory.machineIndex].verb} el sistema y genero energia.`);
...
function factoryWrong() {
  ...
  setFactoryMessage(factoryMode() === "add" ? "Glitch digital! La energia no coincide. Intenta otra suma." : "Glitch digital! Un clon salio inestable. Intenta otra multiplicacion.");
...
function animateAdditionFusion() {
  ...
  els.cloneEquation.querySelectorAll(".addition-clone-set .clone-dot").forEach((clone, index) => {
    clone.style.setProperty("--fusion-delay", `${index * 0.035}s`);
    clone.classList.add("fusion-clone");
  });
...
function updateFactoryHud() {
  ...
  els.factoryMachineText.textContent = `${factory.machineIndex + 1}/${machines.length}`;
...
function renderCloneGroups(groups, items, groupLabel = "Capsula") {
  ...
  for (let groupIndex = 0; groupIndex < groupCount; groupIndex += 1) {
    ...
    label.textContent = `${groupLabel} ${groupIndex + 1}`;
